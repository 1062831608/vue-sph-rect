  ...
  <div class="pay-main">
    <div class="pay-container">
      ...
      <div class="checkout-steps">
        ...
        <div class="submit">
            <el-button type="danger" @click="goPay" class="payBtn">立即支付</el-button>
        </div>
        <div class="otherpay">
          <div class="step-tit">
            <h5>其他支付方式</h5>
...
<!--    微信支付二维码弹出窗口-->
    ...
    >
      <img :src="payImgUrl" alt="" class="payBox">
      <span slot="footer" class="dialog-footer">
    <el-button type="primary" @click="payProblem" size="mini">支付遇见问题</el-button>
  </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods:{
    // 获取订单支付信息
    getOrderInfo() {
      this.$API.apiGetOrderInfo(this.$route.query.orderId).then(res=>{
        if(res.code === 200) {
    ...
    async goPay() {
      ...
      this.payImgUrl = imgUrl
      // 发起连续请求询问服务器是否已经支付成功
      if (!this.timer) {
        this.timer = setInterval(async ()=>{
          let res = await this.$API.apiOrderState(this.orderInfo.orderId)
          this.code = res.code
          if(this.code === 200) {
            // 状态码 200 支付成功跳转路由
    ...
    payProblem() {
      ...
      this.timer = null
      this.dialogVisible = false
      this.$alert('请联系管理员：19936774341','帮助',{
